...
#ifndef _CRYPTO_CRC_CCITTF_TABLE_H_
#define _CRYPTO_CRC_CCITTF_TABLE_H_

static const uint16_t crc_ccittf_table[256] = {
...
static inline uint16_t crc_ccittf_byte(uint16_t crc, const uint8_t byte)
{
    return (crc << 8) ^ crc_ccittf_table[(crc >> 8) ^ byte];
}

static inline uint16_t crc_ccittf_inline(const uint8_t *src, size_t len, uint16_t crc)
{
    for (; len >= 8; len -= 8) {
        crc = crc_ccittf_byte(crc, *src++);
        crc = crc_ccittf_byte(crc, *src++);
        crc = crc_ccittf_byte(crc, *src++);
        crc = crc_ccittf_byte(crc, *src++);
        crc = crc_ccittf_byte(crc, *src++);
...
#endif  /* _CRYPTO_CRC_CCITTF_TABLE_H_ */
