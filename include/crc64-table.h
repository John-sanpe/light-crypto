...
#ifndef _CRYPTO_CRC64_TABLE_H_
#define _CRYPTO_CRC64_TABLE_H_
...
static inline uint64_t crc64_byte(uint64_t crc, const uint8_t data)
{
    uint64_t tmp = ((crc >> 56) ^ data) & 0xff;
    return crc64table[tmp] ^ (crc << 8);
}

static inline uint64_t crc64_inline(const uint8_t *src, int len, uint64_t crc)
{
    for (; len >= 8; len -= 8) {
        crc = crc64_byte(crc, *src++);
        crc = crc64_byte(crc, *src++);
        crc = crc64_byte(crc, *src++);
        crc = crc64_byte(crc, *src++);
        crc = crc64_byte(crc, *src++);
