...
#ifndef _CRYPTO_CRC_ITUT_TABLE_H_
#define _CRYPTO_CRC_ITUT_TABLE_H_

static const uint16_t crc_itut_table[256] = {
...
static inline uint16_t crc_itut_byte(uint16_t crc, const uint8_t data)
{
    return (crc << 8) ^ crc_itut_table[((crc >> 8) ^ data) & 0xff];
}

static inline uint16_t crc_itut_inline(const uint8_t *src, size_t len, uint16_t crc)
{
    for (; len >= 8; len -= 8) {
        crc = crc_itut_byte(crc, *src++);
        crc = crc_itut_byte(crc, *src++);
        crc = crc_itut_byte(crc, *src++);
        crc = crc_itut_byte(crc, *src++);
        crc = crc_itut_byte(crc, *src++);
