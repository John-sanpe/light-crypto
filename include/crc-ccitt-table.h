...
#ifndef _CRYPTO_CRC_CCITT_TABLE_H_
#define _CRYPTO_CRC_CCITT_TABLE_H_
...
static inline uint16_t crc_ccitt_byte(uint16_t crc, const uint8_t byte)
{
    return (crc >> 8) ^ crc_ccitt_table[(crc ^ byte) & 0xff];
}

static inline uint16_t crc_ccitt_inline(const uint8_t *src, size_t len, uint16_t crc)
{
    for (; len >= 8; len -= 8) {
        crc = crc_ccitt_byte(crc, *src++);
        crc = crc_ccitt_byte(crc, *src++);
        crc = crc_ccitt_byte(crc, *src++);
        crc = crc_ccitt_byte(crc, *src++);
        crc = crc_ccitt_byte(crc, *src++);
...
#endif  /* _CRYPTO_CRC_CCITT_TABLE_H_ */
